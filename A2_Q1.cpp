...
public:
    Book(const string& id, const string& title, bool issued = false)
        : id(id), title(title), issued(issued) {}
...
class Library {
    ...
    void loadLibraryData() {
        ...
        if (!file) {
            cout << "No data file found. Starting with an empty library." << endl;
            return;
        }
        ...
        while (getline(file, line)) {
            ...
            string id, title, issued_str;
            getline(ss, id, ',');
            getline(ss, title, ',');
            getline(ss, issued_str, ',');

            bool issued = (issued_str == "1");
